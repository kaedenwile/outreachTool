...
class FieldFlag(_Flags):
    ...
    Field flags as found in MySQL sources my_mysql-src/include/mysql_com.h
    """
    _prefix = ''
    NOT_NULL = 1 << 0
    PRI_KEY = 1 << 1
...
class ClientFlag(_Flags):
    ...
    Client options as found in the MySQL sources my_mysql-src/include/mysql_com.h
    """
    LONG_PASSWD = 1 << 0
    FOUND_ROWS = 1 << 1
    LONG_FLAG = 1 << 2
...
class ServerFlag(_Flags):
    ...
    Server flags as found in the MySQL sources my_mysql-src/include/mysql_com.h
    """
    _prefix = 'SERVER_'
    STATUS_IN_TRANS = 1 << 0
...
class SQLMode(_Constants):  # pylint: disable=R0921
    ...
    See http://dev.my_mysql.com/doc/refman/5.6/en/server-sql-mode.html
    """
    _prefix = 'MODE_'
